    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-4 space-y-0.5">
        {nav.map(({ href, icon: Icon, label }) => {
          const active = pathname === href || (href !== '/dashboard' && href !== '/admin' && href !== '/settings' && pathname.startsWith(href))
          return (
            <Link key={href} href={href} onClick={onMobileClose} className={clsx(active ? 'nav-link-active' : 'nav-link')}>
